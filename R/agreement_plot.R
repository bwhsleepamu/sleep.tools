...
plot_agreement <- function(data, compare_by="method", facet_x_by="age_group", facet_y_by="label", include_flags=c(TRUE),
  ...
  # data <- merge(data, subjects, all.x=TRUE, all.y=FALSE, by='subject_code')
  data <- data[include %in% include_flags,
               ...
               schedule_label %in% schdule_labels,
               sex %in% sexes,
               t_cycle %in% t_cycles]  
  
  plot <- ggplot(data, aes_string(x="agreement", color=compare_by))
  plot <- plot + scale_fill_manual(values=cbbPalette) + scale_colour_manual(values=cbbPalette) + theme_minimal()
  plot <- plot + ggtitle("Agreement within Periods by Method")
  plot <- plot + xlab("Agreement")
  plot <- plot + facet_grid(facet_y_by ~ facet_x_by, scales='free_y')
