...
# Methods
## Method 1
bouts.changepoint.dt <- function(epoch_type, labtime, subject_code) {
  changepoint_results <- processStream(epoch_type, cpmType="Mann-Whitney", ARL0=10000, startup=20)
  # Uses changepoint indeces to create two columns, with start and end indeces for each chunk
  chunks <- as.data.frame(cbind(c(1, changepoint_results$changePoints), c(changepoint_results$changePoints, length(epoch_type))))  
  ...
  bouts <- ddply(chunks, .(start_index, end_index), calculate_bouts, epoch_type, labtime)
  bouts$length <- bouts$end_index - bouts$start_index
  
  bouts <- as.data.table(bouts)
  bouts[,subject_code:=subject_code]
  setkey(bouts, subject_code)
...
# Plotting
plot.periods <- function(sleep_data, periods, selection='changepoint') {  
  # Draw
  .e <- environment()
  # Main Plot
  plot <- ggplot(sleep_data, aes(day_labtime, stage, group=day_number), environment = .e)
  # Faceting
  ...
  plot <- plot + geom_rect(aes(xmin = start_day_labtime, xmax = end_day_labtime + EPOCH_LENGTH, fill = bout_type), ymin = 0, ymax = 10, data = primary_bouts)
